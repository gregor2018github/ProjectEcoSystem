...
import pygame
import config
from animal_arrays import PredatorView
...
class HoverWindow:
    ...
    def __init__(self, animal, anchor_pos: tuple[float, float]) -> None:
        """Initialize the hover window for the given animal.

        Args:
            animal: The animal view to display information about.
        ...
        # Prepare text lines
        self.lines = []
        self.lines.append(f"Type: {'Predator' if is_predator else 'Prey'}")

        max_age_key = config.PREDATOR_MAX_AGE if is_predator else config.PREY_MAX_AGE
        self.lines.append(f"Age: {animal.age} / {max_age_key}")

        max_food_key = config.PREDATOR_MAX_FOOD if is_predator else config.PREY_MAX_FOOD
